...
function limparCachesExecucao() {
  ...
  CACHE_KEYS = [];
  CACHE_DATAS_OBJ = {};
  Logger.log("🧹 Caches de execução limpos.");
}
...
function fila_registrarAlteracao(cpf) {
  ...
  try {
    ...
    Logger.log("Erro Lock Fila: " + e.message + "\n" + e.stack);
  } finally {
...
function trigger_ProcessarFila() {
  ...
  try {
    ...
    Logger.log("Processando: " + cpfsPendentes.length + " CPFs.");
    executarProcessamentoGlobal(false, cpfsPendentes); 

  } catch (e) {
    Logger.log("Erro Trigger: " + e.message + "\nStack: " + e.stack);
  } finally {
...
function executarProcessamentoGlobalForce() {
  ...
  try {
    ...
    Logger.log("Erro Fatal Force: " + e.message + "\nStack: " + e.stack);
  } finally {
    CacheService.getScriptCache().remove("SISTEMA_OCUPADO");
    if (lock.hasLock()) lock.releaseLock();
  }
...
function montarMotivoPadronizado(tipo, dados, layout, textoOriginal) {
  ...
  if (!tipo) Logger.log("⚠️ Motivo não identificado: " + textoOriginal);
